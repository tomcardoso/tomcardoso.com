...
class Exhibits extends Router
{
	...
	function page_section()
	{
		...
		if ($rs['secid'] != 1)
		{
			$check = $this->db->fetchRecord("SELECT id FROM ".PX."objects WHERE section_id = '$go[id]'");
			
			// can only access delete if there are no subpages
			if ($check)
			{
				$body .= "<div style='margin: 18px 0;'>You may only delete this section if there are no exhibits within it.</div>\n";
			}
			else
			{
	...
	function page_view()
	{
		...
		$body .= ips($this->lang->word('image title'), 'input', 'media_title', 
			$rs['media_title'], "id='media_title' maxlength='100'", 'text');
		$body .= ips($this->lang->word('image caption'), 'input', 'media_caption', 
			$rs['media_caption'], "id='media_caption' maxlength='100'", 'text');
	...
	function sbmt_del_sec()
	{
		...
		// we need to check the section first to see if there are dependents
		$check = $this->db->fetchRecord("SELECT id FROM ".PX."objects WHERE section_id = '$go[id]'");
		
		if ($check)
		{
			// don't allow people to do this
			return;
		}
		else
		{
